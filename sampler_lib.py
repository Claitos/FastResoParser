...
def mass_conservation(p_df: pd.DataFrame, d_df: pd.DataFrame, edge_id: str = "+", verbose: bool = False) -> pd.DataFrame:
    ...
    for index, row in d_df.iloc[::order].iterrows():
        ...
        if total_decay_mass > parent_mass:
            ...
            if edge_id == "+":
                scale_factor = parent_mass / total_decay_mass
                ...
                p_df.loc[p_df["ID"] == parent_id, "Mass (GeV)"] = total_decay_mass
...
def edge_study(p_df: pd.DataFrame, d_df: pd.DataFrame, cut: int = 0, verbose: bool = False) -> None:
    ...
    if cut == 0:
        dir_name = "Datafiles_sampled/edge_study_mass"
        p_df_cut = p_df
        d_df_cut = d_df
    else:
        dir_name = f"Datafiles_sampled/edge_study_mass_{cut:.0e}"
    ...
    p_dfs = [p_df_cut, mass_p_df, mass_n_df]
    d_dfs = [d_df_cut, d_df_cut, d_df_cut]
